...
%    coeff_re(q) = fi(coefficients(2*q-1),s,w1,f1);
%    coeff_im(q) = fi(coefficients(2*q),s,w1,f1);
end 

%fi(coefficients(2*q-1),s,w1,f1);
% Reading inputs
FID2 = fopen('fft_inputs_fixed.txt','r');
...
% FFT Reference
for a=1 : 1 : N
    input_complex(a) = input_re(a)+1i*input_im(a);
end
